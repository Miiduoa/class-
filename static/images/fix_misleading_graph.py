...
def update_misleading_graph(font_prop=None):
    """更新誤導性圖表示例，確保中文顯示正確"""
    # 創建一個專業的示範誤導性圖表的視覺化
    ...
    # 添加一些說明文字
    subtitle = '以下展示常見的圖表設計陷阱以及如何識別它們'
    if font_prop:
        plt.figtext(0.5, 0.94, subtitle, 
                   ha='center', fontsize=14, style='italic', fontproperties=font_prop)
    else:
        plt.figtext(0.5, 0.94, subtitle, 
    ...
    if font_prop:
        ax1.set_title('正確表示：完整Y軸比例尺', fontsize=14, fontproperties=font_prop)
        ax1.set_xlabel('年份', fontsize=12, fontproperties=font_prop)
        ax1.set_ylabel('收入 (百萬元)', fontsize=12, fontproperties=font_prop)
    else:
    ...
    if font_prop:
        ax2.set_title('誤導性表示：截斷的Y軸', fontsize=14, fontproperties=font_prop)
        ax2.set_xlabel('年份', fontsize=12, fontproperties=font_prop)
        ax2.set_ylabel('收入 (百萬元)', fontsize=12, fontproperties=font_prop)
    else:
    ...
    warning_text1 = '截斷！'
    warning_text2 = '此處Y軸從95開始，誇大了變化幅度'
    
    if font_prop:
        ax2.text(years[0]-0.5, ax2.get_ylim()[0]+1, warning_text1, 
                color='#e74c3c', fontweight='bold', fontsize=12, fontproperties=font_prop)
        ax2.text((years[0]+years[-1])/2, ax2.get_ylim()[0]+2, warning_text2,
                ha='center', color='#e74c3c', fontsize=10, style='italic', fontproperties=font_prop,
    ...
    if font_prop:
        ax3.set_title('合適的圖表：折線圖展示趨勢', fontsize=14, fontproperties=font_prop)
        ax3.set_xlabel('年份', fontsize=12, fontproperties=font_prop)
        ax3.set_ylabel('收入 (百萬元)', fontsize=12, fontproperties=font_prop)
    else:
    ...
    if font_prop:
        ...
        # 用fontproperties設置餅圖標籤字體
        wedges, texts, autotexts = ax4.pie(revenue, labels=None, autopct='%1.1f%%', 
                            startangle=90, colors=plt.cm.Paired(np.linspace(0,1,len(revenue))))
        ...
        # 單獨添加標籤
        ax4.legend(wedges, labels, loc="center right", bbox_to_anchor=(0.8, 0, 0.5, 1), fontsize=10, prop=font_prop)
        
        ax4.set_title('誤導性表示：圓餅圖不適合時間趨勢', fontsize=14, fontproperties=font_prop)
        
        warning_text = '圓餅圖應用於展示部分與整體關係，不適合時間趨勢！'
        ax4.text(0, -1.3, warning_text,
                ha='center', color='#e74c3c', fontsize=12, style='italic', fontproperties=font_prop,
                bbox=dict(boxstyle='round,pad=0.3', fc='white', ec='#e74c3c', alpha=0.7))
    else:
...
"""
    ...
    plt.savefig('ch3/misleading_graph.png', dpi=120, bbox_inches='tight')
    print(f"已生成誤導性圖表示例：{os.path.abspath('ch3/misleading_graph.png')}")
...
if __name__ == "__main__":
    ...
    # 更新誤導性圖表
    update_misleading_graph(font_prop) 